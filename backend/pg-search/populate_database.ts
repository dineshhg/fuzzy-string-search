...
async function populateDatabase(): Promise<void> {
  ...
  try {
    // Clear existing data
    await pool.query("TRUNCATE TABLE persons RESTART IDENTITY");
    ...
    for (const [first, last] of allTestCases) {
      ...
      const emailLast = last
        ...
        .replace(/[()]/g, "");
      const email = last
        ? `${emailFirst}.${emailLast}@example.com`
        : `${emailFirst}@example.com`;
    ...
    for (let i = 0; i < remaining; i++) {
      ...
      if (Math.random() < 0.85) {
        ...
        if (variationType === "compound") {
          ...
          firstName = faker.person.firstName();
          lastName =
            ["O'", "D'", "L'", "M'"][Math.floor(Math.random() * 4)] +
    ...
    for (let i = 0; i < data.length; i += batchSize) {
      ...
      await pool.query(
        `INSERT INTO persons (first_name, last_name, full_name, email) VALUES ${placeholders}`,
        flatData,
      );
    ...
    console.log(
      `Successfully inserted ${data.length} records into the database`,
    );
  } catch (error) {
...
async function verifyData(): Promise<void> {
  ...
  try {
    ...
    const randomResult = await pool.query(
      "SELECT * FROM persons ORDER BY RANDOM() LIMIT 10",
    );
    console.log("\nRandom sample records:");
    console.log("-".repeat(80));
    console.log(
      `${"ID".padEnd(5)} ${"First Name".padEnd(20)} ${"Last Name".padEnd(20)} ${"Full Name".padEnd(30)}`,
    );
    console.log("-".repeat(80));

    for (const row of randomResult.rows) {
      console.log(
        `${row.id.toString().padEnd(5)} ${row.first_name.padEnd(20)} ${row.last_name.padEnd(20)} ${row.full_name.padEnd(30)}`,
      );
    }
  } catch (error) {
...
async function main(): Promise<void> {
  console.log(
    "Populating PostgreSQL database with 10,000 sample entries including test cases...",
  );
